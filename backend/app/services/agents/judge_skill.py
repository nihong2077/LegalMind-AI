...
注意：evidence_needed字段判断标准——作为法官，你需要自主判断当前证据是否足以作出裁判。
当存在以下情形之一时，应将evidence_needed设为true，并在evidence_gap_description中具体说明：
1. 关键事实缺乏基本证据支撑：如主张合同违约但未提供合同存在的证据、主张侵权损害但无损害事实证明
2. 核心法律要件事实完全无法认定：如违约时间、损害金额、因果关系等关键要素完全缺失
3. 双方陈述存在重大矛盾且无任何证据可资辨别真伪，导致事实无法认定
4. 举证责任方未提供任何证据支持其核心主张，且对方予以否认

以下情形不应设为true（应通过辩论过程自行厘清）：
- 一般性的证据薄弱或论证不足
- 双方对事实有争议但各有说辞且有部分证据
- 仅是法律适用存在争议（不涉及事实认定）
- 可以通过证据优势原则作出判断的情形

evidence_gap_description要求：具体说明缺失的是什么证据、需要证明什么事实、应由哪方补充。"""
