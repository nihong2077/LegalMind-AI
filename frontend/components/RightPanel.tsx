...
export default function RightPanel({ onQuickQuestionClick }: RightPanelProps) {
  ...
  return (
    <aside className="w-72 border-l border-gray-200 flex-shrink-0 overflow-y-auto bg-slate-50">
      <div className="p-4 space-y-4">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-700">快捷问题</span>
            </div>
        ...
        <section className="pt-3 border-t border-gray-200">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-700">常见场景</span>
            </div>
            ...
            {SCENARIOS.map((s, i) => (
              <button key={i} onClick={() => onQuickQuestionClick?.(s.desc)}
                ...
                <div className="min-w-0 flex-1">
                  <p className={`text-[11px] font-medium truncate ${scenarioType && s.keywords.includes(scenarioType) ? 'text-emerald-600' : 'text-slate-600 group-hover:text-slate-800'}`}>
                    {s.label}
                  </p>
        ...
        <section className="pt-3 border-t border-gray-200">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-semibold text-slate-700">推荐下一步</span>
          </div>
          <div className="space-y-1.5">
            {steps.map((step, i) => (
              <button key={i} onClick={() => onQuickQuestionClick?.(step)}
                className="w-full flex items-start gap-2 p-2 rounded-lg hover:bg-white text-left group transition-all">
                <span className="w-4 h-4 rounded-full bg-amber-50 text-amber-600 text-[9px] flex items-center justify-center flex-shrink-0 mt-0.5">{i + 1}</span>
                <span className="text-[10px] text-slate-500 leading-relaxed group-hover:text-slate-700 line-clamp-2 flex-1">{step}</span>
        ...
        {(riskWarnings.length > 0 || true) && (
          <section className="pt-3 border-t border-gray-200">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-slate-700">风险提示</span>
            </div>
            {riskWarnings.length > 0 ? riskWarnings.map((risk, i) => (
              ...
              <div className="rounded-lg bg-amber-50 border border-amber-200 p-2.5">
                <p className="text-[10px] text-slate-500 leading-relaxed">⚠️ 法院时效一般为3年，自权利人知道或应当知道权利被侵害时起算。请及时主张权利，避免超过诉讼时效。</p>
              </div>
            )}
          </section>
        )}
        ...
        <section className="pt-3 border-t border-gray-200">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-700">相关法条</span>
            </div>
            <button className="flex items-center gap-0.5 text-[10px] text-blue-600 hover:text-blue-700">
              更多 <ChevronRight size={10} />
            </button>
          ...
          {relatedLaws.length > 0 ? relatedLaws.map((law, i) => (
            <div key={i} className="rounded-lg bg-purple-50 border border-purple-200 p-2.5">
              <p className="text-[10px] text-slate-600 leading-relaxed">{law}</p>
            </div>
          )) : (
            <div className="space-y-1.5">
              {[
                ...
                <div key={i} className="rounded-lg bg-slate-50 border border-gray-200 p-2">
                  <p className="text-[10px] text-slate-500 truncate">{law}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <section className="pt-3 border-t border-gray-200">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-700">法律术语解释</span>
            </div>
            <button className="flex items-center gap-0.5 text-[10px] text-blue-600 hover:text-blue-700">
              更多 <ChevronRight size={10} />
            </button>
          ...
          {termExplanations.length > 0 ? termExplanations.map((t, i) => (
            <div key={i} className="rounded-lg bg-cyan-50 border border-cyan-200 p-2.5">
              ...
              <p className="text-[10px] text-slate-500 leading-relaxed">{t.explanation}</p>
            </div>
          )) : (
            <div className="rounded-lg bg-slate-50 border border-gray-200 p-2.5">
              <p className="text-[10px] text-slate-500 leading-relaxed">在民事法律过程中，因各种老化或疏忽导致使用过期的证据。</p>
            </div>
          )}
        </section>
        ...
        <section className="pt-3 border-t border-gray-200">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-semibold text-slate-700">合规指南</span>
          </div>
      ...
      {/* 收起按钮 */}
      <div className="sticky bottom-0 pt-3 pb-4 px-4 border-t border-gray-200 bg-white">
        <button onClick={() => setActiveSection(null)}
          className="w-full flex items-center justify-center gap-1 text-[10px] text-slate-400 hover:text-slate-600 transition-colors py-1">
